...
final class HybridBinarizer extends GlobalHistogramBinarizer{
	...
	public function getBlackMatrix():BitMatrix{
		...
		if($width >= self::MINIMUM_DIMENSION && $height >= self::MINIMUM_DIMENSION){
			...
			$this->matrix = $this->calculateThresholdForBlock($luminances, $subWidth, $subHeight, $width, $height, $blackPoints);
		}
		else{
	 ...
	 */
	private function calculateBlackPoints(
		array $luminances,
		int $subWidth,
		int $subHeight,
		int $width,
	...
	public function createBinarizer(LuminanceSource $source):Binarizer{
